...
export function buildBoarGroup(level) {
  ...
  const frameW = Number(level.assets?.boarFrameW) || Number(tiles.frameW) || 32;
  const frameH = Number(level.assets?.boarFrameH) || Number(tiles.frameH) || 32;
  const offsetY = Number(
    level.tuning?.boar?.aniOffsetY ??
      level.tuning?.boar?.anisOffset?.y ??
      -8,
  );
  ...
  if (hasDefs) {
    ...
    safeConfigureAniSheet(level.boar, frameW, frameH, offsetY);
...
function ensureBoarAnis(level, e) {
  ...
  safeConfigureAniSheet(e, frameW, frameH, offsetY);
