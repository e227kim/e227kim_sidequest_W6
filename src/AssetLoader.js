...
export async function loadAssets(levelPkg, tuningDoc) {
  ...
  // Player spritesheet layout (columns/rows). Defaults match current sheet.
  const playerSheetCols = Number(tuningDoc?.player?.sheetCols ?? 6);
  const playerSheetRows = Number(tuningDoc?.player?.sheetRows ?? 6);
  const playerFrameW =
    Number.isFinite(playerSheetCols) && playerSheetCols > 0
      ? Math.floor(playerImg.width / playerSheetCols)
      : undefined;

  // Boar spritesheet layout (columns/rows). Defaults match current sheet.
  const boarSheetCols = Number(tuningDoc?.boar?.sheetCols ?? 8);
  const boarSheetRows = Number(tuningDoc?.boar?.sheetRows ?? 4);
  const boarFrameW =
    Number.isFinite(boarSheetCols) && boarSheetCols > 0
      ? Math.floor(boarImg.width / boarSheetCols)
      : undefined;
  const boarFrameH =
    Number.isFinite(boarSheetRows) && boarSheetRows > 0
      ? Math.floor(boarImg.height / boarSheetRows)
      : undefined;
  const playerFrameH =
    Number.isFinite(playerSheetRows) && playerSheetRows > 0
      ? Math.floor(playerImg.height / playerSheetRows)
      : undefined;
...
function defaultPlayerAnis() {
  return {
    ...
    run: { row: 5, frames: 6, frameDelay: 3 },
    jump: { row: 4, frames: 4, frameDelay: Infinity, frame: 2 },
    attack: { row: 0, frames: 3, frameDelay: 2 },
    hurtPose: { row: 2, frames: 4, frameDelay: Infinity, frame: 1 },
    death: { row: 1, frames: 4, frameDelay: 16 },
  };
}

function defaultBoarAnis() {
  return {
    run: { row: 3, frames: 4, frameDelay: 3 },
    throwPose: { row: 0, frames: 4, frameDelay: Infinity, frame: 1 },
    death: { row: 0, frames: 4, frameDelay: 16 },
  };
}
