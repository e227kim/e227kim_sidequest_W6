...
export class PlayerEntity {
  constructor(pkg, assets) {
    ...
    this.ANI_OFFSET_Y = Number(
      this.tuning.player?.aniOffsetY ??
        this.tuning.player?.anisOffset?.y ??
        -8,
    );
  ...
  // -----------------------
  buildSprites() {
    let frameW = Number(this.assets?.playerFrameW);
    let frameH = Number(this.assets?.playerFrameH);
    if (!Number.isFinite(frameW) || frameW <= 0) {
      frameW = Number(this.tilesCfg.frameW ?? 32);
    }
    if (!Number.isFinite(frameH) || frameH <= 0) {
      frameH = Number(this.tilesCfg.frameH ?? 32);
    }
